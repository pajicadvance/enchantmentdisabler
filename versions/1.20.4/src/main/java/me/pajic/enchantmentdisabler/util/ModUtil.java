...
public class ModUtil {
    ...
    public static Enchantment rerollEnchantmentIfBlacklisted(ItemStack stack, Enchantment enchantment, RandomSource random) {
        if (Main.CONFIG.disabledEnchantments().contains(BuiltInRegistries.ENCHANTMENT.getKey(enchantment).toString())) {
            List<Enchantment> compatibleEnchantments = BuiltInRegistries.ENCHANTMENT.stream().filter(e ->
                    e.canEnchant(stack) && !Main.CONFIG.disabledEnchantments().contains(BuiltInRegistries.ENCHANTMENT.getKey(e).toString())
            ).toList();
            ...
            return compatibleEnchantments.get(random.nextInt(compatibleEnchantments.size()));
        }
        return enchantment;
    }
}
